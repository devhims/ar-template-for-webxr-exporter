(function () {
  ...
  XRManager.prototype.onRequestARSession = function () {
    ...
    navigator.xr
      .requestSession('immersive-ar', {
        requiredFeatures: this.gameModule.WebXR.Settings
          .ARRequiredReferenceSpace,
  ...
  XRManager.prototype.onEndSession = function (xrSessionEvent) {
    ...
    document.dispatchEvent(
      new CustomEvent('XRControllersData', {
        detail: {
    ...
    this.canvas.width =
      ...
      this.gameModule.asmLibraryArg._JS_SystemInfo_GetPreferredDevicePixelRatio();
    this.canvas.height =
  ...
  XRManager.prototype.onInputSourceEvent = function (xrInputSourceEvent) {
    if (
      ...
      if (
        ...
        xPercentage =
          (xrInputSourceEvent.inputSource.gamepad.axes[0] + 1.0) * 0.5;
        yPercentage =
          (xrInputSourceEvent.inputSource.gamepad.axes[1] + 1.0) * 0.5;
      }
      switch (xrInputSourceEvent.type) {
    ...
    if (gameModule && !this.gameModule) {
      ...
      this.BrowserObject.requestAnimationFrame = function (func) {
        if (thisXRMananger.xrSession && thisXRMananger.xrSession.isInSession) {
          return thisXRMananger.xrSession.requestAnimationFrame(
            (time, xrFrame) => {
      ...
      this.ctx.bindFramebuffer = ((oldBindFramebuffer) =>
        function bindFramebuffer(target, fbo) {
          if (!fbo) {
            if (
  ...
  XRManager.prototype.onSessionStarted = function (session) {
    ...
    if (session.isImmersive) {
      ...
      if (session.isAR) {
        refSpaceType = this.gameModule.WebXR.Settings
          .ARRequiredReferenceSpace[0];
      }
  ...
  XRManager.prototype.animate = function (frame) {
    ...
    if (session.isAR && this.viewerHitTestSource) {
      ...
      let viewerHitTestResults = frame.getHitTestResults(
        this.viewerHitTestSource
      );
      if (viewerHitTestResults.length > 0) {
        let hitTestPose = viewerHitTestResults[0].getPose(
        ...
        xrData.viewerHitTestPose.position[1] =
          hitTestPose.transform.position.y +
          Math.abs(
            hitTestPose.transform.position.y -
              hitTestPoseBase.transform.position.y
          );
        xrData.viewerHitTestPose.position[2] = -hitTestPose.transform.position
          .z;
        xrData.viewerHitTestPose.rotation[0] = -hitTestPose.transform
          .orientation.x;
        xrData.viewerHitTestPose.rotation[1] = -hitTestPose.transform
          .orientation.y;
        xrData.viewerHitTestPose.rotation[2] =
          hitTestPose.transform.orientation.z;
        xrData.viewerHitTestPose.rotation[3] =
          hitTestPose.transform.orientation.w;
      } else {
        xrData.viewerHitTestPose.available = 0;
      }
      document.dispatchEvent(
        new CustomEvent('XRViewerHitTestPose', {
          detail: {
